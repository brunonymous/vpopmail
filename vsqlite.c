...
, in_domain
    ...
    if (rc != SQLITE_ROW) {
        ...
        if ( sqlite3_column_text(stmt_read, 7) != 0 ) {
            strncpy(vpw.pw_clear_passwd, sqlite3_column_text(stmt_read, 7),SMALL_BUFF);
        }
#endif
    }
...
struct vqpasswd *vauth_getall(char *domain, int first, int sortit)
{
    ...
    if (rc == SQLITE_ROW) {
        ...
        if (sqlite3_column_text(stmt_getall, 7)!=0) {
            strncpy(vpw.pw_clear_passwd, sqlite3_column_text(stmt_getall, 7),SMALL_BUFF);
        }
#endif
        more = 1;
...
where alias = '%s' and domain = '%s'", alias, domain );
    ...
    while (rc == SQLITE_OK) {
        ...
        temp_entry = linklist_add (temp_entry, sqlite3_column_text(stmt_read, 0), "");
...
int vget_limits(const char *domain, struct vlimits *limits)
{
    ...
    if (rc == SQLITE_ROW) {
        ...
        limits->defaultmaxmsgcount = strtoll(sqlite3_column_text(stmt_read, 8), NULL, 10);
        ...
        limits->perm_defaultquota = strtoll(sqlite3_column_text(stmt_read, 24), NULL, 10);
    } else 
    if (rc == SQLITE_DONE) {    
        sqlite3_finalize(stmt_read);
